...
namespace webserver {
    multi_thread_tcp_server::multi_thread_tcp_server(unsigned short int PORT, const function<bool(string)>& is_full_message,
    ...
    void multi_thread_tcp_server::start() {
        ...
        for (unsigned int thread_num = 1; thread_num <= MAX_EPOLL_THREADS; thread_num++) {
            ...
            thread listen_events_thread(&multi_thread_tcp_server::listen_events, this, EPoll, thread_num);
    ...
    void multi_thread_tcp_server::set_client(int client_sock) {
        uid++;
    ...
    void multi_thread_tcp_server::accept_connections() {
        while (accept_allow) {
    ...
    void multi_thread_tcp_server::listen_events(int EPoll, int thread_num) {
    ...
    void multi_thread_tcp_server::stop() {
        accept_allow = false;
